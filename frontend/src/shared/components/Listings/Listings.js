...
export const Listings = () =>{
	...
	async function onSubmit(e) {
		...
		fetch("http://localhost:5000/api/houses/search/" + form.searchType + "/" + form.searchParam)
		.then(async(res) => await res.json())
		.then((json) => {
			updateSearch({items: json})
			// console.log(json) 
		})
	}
											...
											<Row className='d-flex align-items-center'>
												<Col>
													...
													{/* controlId="destination" onChange={handleChange} value={this.state.destination} */}
														<Form.Select name="city" onChange={(e) => updateSearch({searchParam: e.target.value})}>
															<option></option>
